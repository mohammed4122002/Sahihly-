...
import { humanizerUseCases, getHumanizerUseCase } from "@/content/humanizer-use-cases";
...
export function generateStaticParams() {
  return humanizerUseCases.flatMap((u) =>
...
export async function generateMetadata({
  ...
  const u = getHumanizerUseCase(useCase);
  if (!u) return {};
  const c = u[loc];
  return {
    title: c.metaTitle,
    description: c.metaDescription,
    alternates: alternatesFor(loc, `/ai-humanizer/${useCase}`),
    openGraph: {
      images: [
        {
...
export default async function HumanizerUseCasePage({
  params,
}: {
  params: Promise<{ locale: string; useCase: string }>;
}) {
    ...
    },
    {
      ...
      itemListElement: [
        { "@type": "ListItem", position: 1, name: "Sahihly", item: pageUrl(locale, "/") },
        {
          "@type": "ListItem",
          position: 2,
          name: ar ? "مُنسّن الذكاء الاصطناعي" : "AI Humanizer",
          item: pageUrl(locale, "/ai-humanizer"),
        },
        {
          "@type": "ListItem",
          position: 3,
          name: c.h1,
          item: pageUrl(locale, `/ai-humanizer/${useCase}`),
        },
      ],
    },
  ];
    ...
    <div className="py-14">
      ...
      <div className="container-x">
        <Reveal>
          ...
          >
            <TreeLink href="/ai-humanizer" className="hover:text-white/70">
              {ar ? "مُنسّن الذكاء الاصطناعي" : "AI Humanizer"}
            </TreeLink>
