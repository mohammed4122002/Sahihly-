...
import { readabilityUseCases, getReadabilityUseCase } from "@/content/readability-use-cases";
...
export function generateStaticParams() {
  return readabilityUseCases.flatMap((u) =>
...
export async function generateMetadata({
  ...
  const u = getReadabilityUseCase(useCase);
  if (!u) return {};
  const c = u[loc];
  return {
    title: c.metaTitle,
    description: c.metaDescription,
    alternates: alternatesFor(loc, `/readability-checker/${useCase}`),
    openGraph: {
      images: [
        {
...
export default async function ReadabilityUseCasePage({
  params,
}: {
  params: Promise<{ locale: string; useCase: string }>;
}) {
    ...
    },
    {
      ...
      itemListElement: [
        { "@type": "ListItem", position: 1, name: "Sahihly", item: pageUrl(locale, "/") },
        {
          "@type": "ListItem",
          position: 2,
          name: ar ? "فاحص سهولة القراءة" : "Readability Checker",
          item: pageUrl(locale, "/readability-checker"),
        },
        {
          "@type": "ListItem",
          position: 3,
          name: c.h1,
          item: pageUrl(locale, `/readability-checker/${useCase}`),
        },
      ],
    },
  ];
    ...
    <div className="py-14">
      ...
      <div className="container-x">
        <Reveal>
          ...
          >
            <TreeLink href="/readability-checker" className="hover:text-white/70">
              {ar ? "فاحص سهولة القراءة" : "Readability Checker"}
            </TreeLink>
          ...
          <div className="mt-14 flex flex-wrap justify-center gap-3">
            {readabilityUseCases
              ...
              .map((x) => (
                <TreeLink
                  key={x.slug}
                  href={`/readability-checker/${x.slug}`}
