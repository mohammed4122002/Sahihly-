...
import { wordCounterUseCases, getWordCounterUseCase } from "@/content/word-counter-use-cases";
...
export function generateStaticParams() {
  return wordCounterUseCases.flatMap((u) =>
...
export async function generateMetadata({
  ...
  const u = getWordCounterUseCase(useCase);
  if (!u) return {};
  const c = u[loc];
  return {
    title: c.metaTitle,
    description: c.metaDescription,
    alternates: alternatesFor(loc, `/word-counter/${useCase}`),
    openGraph: {
      images: [
        {
...
export default async function WordCounterUseCasePage({
  params,
}: {
  params: Promise<{ locale: string; useCase: string }>;
}) {
    ...
    },
    {
      ...
      itemListElement: [
        { "@type": "ListItem", position: 1, name: "Sahihly", item: pageUrl(locale, "/") },
        {
          "@type": "ListItem",
          position: 2,
          name: ar ? "عدّاد الكلمات" : "Word Counter",
          item: pageUrl(locale, "/word-counter"),
        },
        {
          "@type": "ListItem",
          position: 3,
          name: c.h1,
          item: pageUrl(locale, `/word-counter/${useCase}`),
        },
      ],
    },
  ];
    ...
    <div className="py-14">
      ...
      <div className="container-x">
        <Reveal>
          ...
          >
            <TreeLink href="/word-counter" className="hover:text-white/70">
